...
public class multi4 extends JFrame {
    ...
    static {
        ...
        USER_DB.put("student", "geu123");
        USER_DB.put("alice",   "alice123");
        USER_DB.put("bob",     "bob123");

        ROLE_DB.put("admin",   "admin");
        ROLE_DB.put("student", "student");
        ROLE_DB.put("alice",   "student");
        ROLE_DB.put("bob",     "student");

        // Seed sample data so UI looks alive immediately
        COMPLAINTS.add(new Complaint("student", "Hostel water supply irregular since Monday.", "Hostel",  "High"));
        COMPLAINTS.add(new Complaint("alice",   "Library books not returned on time.",          "Library", "Medium"));
        COMPLAINTS.add(new Complaint("bob",     "Canteen food quality has dropped significantly.", "Canteen", "Low"));
        COMPLAINTS.get(1).setStatus("Accepted");
        COMPLAINTS.get(2).setStatus("Rejected");
    }
    ...
    void showLoginScreen(String expectedRole) {
        ...
        loginBtn.addActionListener(e -> {
            ...
            if (!USER_DB.containsKey(u) || !USER_DB.get(u).equals(p)) {
                shake(box);
                JOptionPane.showMessageDialog(this, "Invalid credentials.", "Login Failed", JOptionPane.ERROR_MESSAGE);
                return;
            }
            String role = ROLE_DB.getOrDefault(u, "student");
            if ("admin".equals(expectedRole) && !"admin".equals(role)) {
                shake(box);
                JOptionPane.showMessageDialog(this, "Access denied — not an admin account.", "Role Mismatch", JOptionPane.WARNING_MESSAGE);
                return;
            }
            sessionUser = u; sessionRole = role;
    ...
    void showRegisterScreen() {
        ...
        regBtn.addActionListener(e -> {
            ...
            USER_DB.put(u, p); ROLE_DB.put(u, "student");
    ...
    void showStudentDashboard() {
        ...
        String[] pris = {"Low","Medium","High","Critical"};
        JComboBox<String> catBox = styledCombo(cats);
        JComboBox<String> priBox = styledCombo(pris);
        ...
        form.add(fieldLbl("Priority"));     form.add(Box.createVerticalStrut(5));
        form.add(priBox);                   form.add(Box.createVerticalStrut(20));
        form.add(submitBtn);
        ...
        submitBtn.addActionListener(e -> {
            ...
            Complaint c = new Complaint(sessionUser, txt,
                (String)catBox.getSelectedItem(), (String)priBox.getSelectedItem());
            COMPLAINTS.add(c);
            area.setText("");
    ...
    JPanel studentCard(Complaint c) {
        ...
        topRow.add(lbl("#"+c.getId()+"  ["+c.getCategory()+"]  Priority: "+c.getPriority(), F_SMALL, C_TEXT_DIM), BorderLayout.WEST);
...
JPanel adminTableHeader() {
    ...
    for (int i = 0; i < cols.length; i++) {
        ...
        JLabel h = new JLabel(cols[i], JLabel.CENTER);
        h.setFont(F_SMALL);
   ...
   JPanel adminRow(Complaint c, Runnable onAction) {
    GlassPanel row = new GlassPanel(new GridBagLayout(), 14);
   ...
   double[] weights = COL_WEIGHTS;
    ...
    // ID
    gc.gridx = 0; gc.weightx = weights[0];
    ...
    // USER
    gc.gridx = 1; gc.weightx = weights[1];
    JLabel user = new JLabel(c.getUser(), JLabel.CENTER);
    user.setFont(F_BODY);
    user.setForeground(C_TEXT);
    row.add(wrap(user), gc);

    // CATEGORY
    gc.gridx = 2; gc.weightx = weights[2];
    JLabel cat = new JLabel(c.getCategory(), JLabel.CENTER);
    cat.setFont(F_SMALL);
    cat.setForeground(C_TEXT_DIM);
    row.add(wrap(cat), gc);

    // STATUS
    gc.gridx = 3; gc.weightx = weights[4];
   JPanel statusPanel = new JPanel(new GridBagLayout());
statusPanel.setOpaque(false);
statusPanel.add(statusBadge(c.getStatus()));
row.add(wrap(statusPanel), gc);

    // TEXT (left aligned for readability)
    gc.gridx = 4; gc.weightx = weights[5];
    String prev = c.getText().length() > 68
        ? c.getText().substring(0, 65) + "..."
        : c.getText();

    JLabel txt = new JLabel(prev);
    txt.setFont(F_BODY);
    txt.setForeground(new Color(195,208,235));
    txt.setHorizontalAlignment(JLabel.LEFT);
    row.add(wrap(txt), gc);

    // ACTIONS
    gc.gridx = 5; gc.weightx = weights[6];
    row.add(wrap(adminActions(c, onAction)), gc);
...
return bar;
    ...
    static class Complaint {
        ...
        Complaint(String user, String text) { this.id=COUNTER++; this.user=user; this.text=text; }
        Complaint(String user, String text, String cat, String pri) {
            this(user, text); this.category=cat; this.priority=pri;
        }
        ...
        String getAdminNote() { return adminNote; }

        void setStatus(String s)    { status    = s; }
        void setCategory(String s)  { category  = s; }
        void setPriority(String s)  { priority  = s; }
        void setAdminNote(String s) { adminNote = s; }
    }
